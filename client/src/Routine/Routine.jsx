...
function Routine() {
  ...
  const sendCourseData = async (course) => {
    try {
      await axios.post('/auth/api/courseSelected', {
      ...
      courseArray.forEach(course => {
        ...
        if (tableCell1) {
          if (tableCell1.textContent.trim() !== "") {
            tableCell1.innerHTML += `, <span className="red-text">${CourseName} Section ${course.Section}</span>`;
          } else {
        ...
        if (tableCell2) {
          if (tableCell2.textContent.trim() !== "") {
            tableCell2.innerHTML += `, <span className="red-text">${CourseName} Section ${course.Section}</span>`;
          } else {
  ...
  const getTableCell = (Time, Day) => {
    switch (Day) {
      case "Sunday":
        return document.querySelector(`td[data-time="${Time}"] + td[data-day="Sunday"]`);
      case "Monday":
        return document.querySelector(`td[data-time="${Time}"] + td + td[data-day="Monday"]`);
      case "Tuesday":
        return document.querySelector(`td[data-time="${Time}"] + td + td + td[data-day="Tuesday"]`);
      case "Wednesday":
        return document.querySelector(`td[data-time="${Time}"] + td + td + td + td[data-day="Wednesday"]`);
      case "Thursday":
        return document.querySelector(`td[data-time="${Time}"] + td + td + td + td + td[data-day="Thursday"]`);
      case "Friday":
        return document.querySelector(`td[data-time="${Time}"] + td + td + td + td + td + td[data-day="Friday"]`);
      case "Saturday":
        return document.querySelector(`td[data-time="${Time}"] + td + td + td + td + td + td + td[data-day="Saturday"]`);
      default:
        return null;
    }
  };
  ...
  return (
    <div>
      <div className="header">
        ...
        <button id="customize-button" onClick={() => window.location.href = "/"}>Home</button>
      </div>
      ...
      <div id="search-container">
        <input type="text" id="course-search" placeholder="Search for a course..." onInput={showSearchResults} />
